...
class MaxLikeHMM:
    ...
    #         for j in range(hidden):
    #
    #             prob[j,i] = np.max(prob[:, i-1] * transition[:, j] * normal_pdf(obs[i], emission[j, 0], emission[j, 1]))
    #             state[j,i] = np.argmax(prob[:, i-1] * transition[:, j] * normal_pdf(obs[i], emission[j, 0], emission[j, 1]))
    #
    #     path[d-1] = np.argmax(prob[:, d-1])
    #     for i in range(d-2, -1, -1):
    #         path[i] = state[path[i+1], [i+1]]
